...
# define WAVE_MAKER_OBLIQUE
        wp=13.0           ! period
        wa=0.24           ! amplitude
        wd=0.0            ! incidence angle (deg)
        wds=10.0          ! directional spread (deg)
                          !  -> crest length = wl/(2*sin(wds))
        gamma=3.3         ! JONSWAP peakedness parameter
# if defined WAVE_MAKER_DSPREAD && defined NS_PERIODIC
!   correct wave directions for periodicity
#  define WAVE_MAKER_DSPREAD_PER   
# endif
#elif defined SWASH
...
!
          ...
          do jw=1,Nfrq
            ...
            if (wd_bry(jw).le.0.0) wd_bry(jw)=wd_bry(jw)+Ndir
...
!
!          open (unit=13,file='wd_bry.txt',status='replace')
!          open (unit=14,file='wa_bry_d.txt',status='replace')
!          open (unit=15,file='wf_bry.txt',status='replace')
!          open (unit=16,file='wa_bry.txt',status='replace')
          cff3=2*pi/el     ! domain wavenumber 
          diff=0.0
          do jw=1,Nfrq
!            write (13,*) wd_bry(jw)
!            write (14,*) wa_bry_d(jw)
!            write (15,*) wf_bry(jw)
!            write (16,*) wa_bry(jw)
...
!              wd_bry(jw)=0.0
            ...
            diff = diff + abs((cff6 - wd_bry(jw))/cff6)
            !if ((cff6.eq.wd).AND.(abs(wf_bry(jw)-wf).lt.0.01)) then
            !  write (*,*) "WAVEMAKER: Mean wave angle has been modified from"
            !  write (*,*) cff6*rad2deg,"deg. to ",wd_bry(jw)*rad2deg,"deg. at T=",2.*pi/wf_bry(jw), "s"
            !endif
            !write (13,*) wd_bry(jw)
            !write (14,*) wa_bry_d(jw)
            !write (15,*) wf_bry(jw)
          ...
          enddo
          diff = (diff/Nfrq)*100.
          write (*,*) "WAVEMAKER: Wave directions have been modified",
     &      " of an average of ",diff,"%"
